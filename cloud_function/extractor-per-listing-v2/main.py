...
DEALER_RE     = re.compile(r"\b(finance|financing|credit)\b", re.IGNORECASE)
VIN_RE        = re.compile(r"\b[A-Z0-9]{17}\b")
TRANSMISSION_RE = re.compile(r"\b(automatic|manual|stick)\b", re.IGNORECASE)
...
def parse_listing(text: str) -> dict:
    ...
    # Dealer or consumer sale
    dealer = DEALER_RE.search(text)
    if dealer:
        d["dealer sale"] = "Yes"
    else:
        d["dealer sale"] = "No"
    ...
    if vin:
        d["VIN Number"] = vin.group()

    
    # Transmission type
    trans = TRANSMISSION_RE.search(text)
    if trans:
        d["Transmission Type"] = trans.group()
...
#--------------------- Backlog Processing Helpers ---------------------

# Can change output directory here if more extractor copies are made.
out_subdir = "jsonl_v2"

def _success_blob_name(run_id: str, out_subdir: str) -> str:
    return f"{STRUCTURED_PREFIX}/run_id={run_id}/{out_subdir}/_SUCCESS"

def _write_success_marker(run_id: str, out_subdir: str):
    bucket = storage_client.bucket(BUCKET_NAME)
    blob = bucket.blob(_success_blob_name(run_id, out_subdir))
    blob.upload_from_string("", content_type="text/plain")


def _run_ids_with_success_markers(bucket_name: str, structured_prefix: str, out_subdir: str) -> set[str]:
    """
    Return run_ids that have success flags.
    """
    bucket = storage_client.bucket(bucket_name)
    prefix = f"{structured_prefix}/run_id="
    suffix = f"/{out_subdir}/_SUCCESS"
    done: set[str] = set()
    ...
    for blob in bucket.list_blobs(prefix=prefix):
        ...
        if not name.endswith(suffix):
            continue
        try:
            after = name.split("run_id=", 1)[1]
            run = after.split("/", 1)[0]
            if RUN_ID_ISO_RE.match(run) or RUN_ID_PLAIN_RE.match(run):
                done.add(run)
        except Exception:
            continue
...
def _process_run(run_id: str, max_files: int = 0, overwrite: bool = False, out_subdir: str = out_subdir) -> dict:
    scraped_at_iso = _parse_run_id_as_iso(run_id)

    txt_blobs = _txt_objects_for_run(run_id)
    if not txt_blobs:
        return {"run_id": run_id, "ok": False, "error": "no .txt files found for run", "processed_txt": 0}
    ...
    for name in txt_blobs:
        try:
            ...
            out_key = f"{STRUCTURED_PREFIX}/run_id={run_id}/{out_subdir}/{post_id}.jsonl"
    ...
    ok = (errors == 0)
    if ok:
        _write_success_marker(run_id, out_subdir)
...
def extract_http(request: Request):
    """
    Added ability to manually run the extractor to go over the entire backlog of scrapes (process_missing=true):
    Added a max_runs field so as to not timeout Google Cloud when going over the backlog
    """
    ...
    process_missing = bool(body.get("process_missing") or False)

    # backlog controls (only used when process_missing=true)
    max_runs = int(body.get("max_runs") or 0)   # user will set manually
    max_files = int(body.get("max_files") or 0) # 0 = all (50)
    overwrite = bool(body.get("overwrite") or False)  # should remain False for resume
    ...
    if process_missing:
        ...
        done = _run_ids_with_success_markers(BUCKET_NAME, STRUCTURED_PREFIX, out_subdir)
        missing_all = [r for r in runs if r not in done]
        missing_before = len(missing_all)

        # Batch protection: process only first N missing runs (user supplies max_runs)
        targets = missing_all
        if max_runs > 0:
            targets = missing_all[:max_runs]

        processed_ok = []
        processed_failed = []

        for r in targets:
            res = _process_run(r, max_files=max_files, overwrite=overwrite, out_subdir=out_subdir)

            if res.get("ok"):
                processed_ok.append(r)
            else:
                processed_failed.append(res)

        result = {
            "ok": True,
            "mode": "backlog",
            "version": "extractor-jsonl-v2-flex",
            "max_runs": max_runs,
            "runs_remaining_estimate": max(0, missing_before - len(targets)),
            "processed_run_ids": processed_ok
        }
    ...
    # -------------------------
    # Automatic default flow: newest run only
    # -------------------------
    runs = _list_run_ids(BUCKET_NAME, SCRAPES_PREFIX)
    if not runs:
        return jsonify({"ok": False, "error": f"no run_ids found under {SCRAPES_PREFIX}/"}), 200
    ...
    newest = runs[-1]
    res = _process_run(newest, max_files=max_files, overwrite=overwrite, out_subdir=out_subdir)
