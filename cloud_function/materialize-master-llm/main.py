...
# Reads:  gs://<bucket>/<STRUCTURED_PREFIX>/run_id=*/jsonl_llm_jf/*.jsonl
...
CSV_COLUMNS = [
    ...
    "transmission", "drivetrain", "color", "sunroof", "retailer", "vin", "title_status", 
    "llm_provider","llm_model","llm_ts", "source_txt"
]
...
def _jsonl_records_for_run(bucket: str, structured_prefix: str, run_id: str):
    """Yield dict records from .jsonl_llm_jf under .../run_id=<run_id>/json_llm_jf/ (one JSON per file)."""
    b = storage_client.bucket(bucket)
    prefix = f"{structured_prefix}/run_id={run_id}/jsonl_llm_jf/"
